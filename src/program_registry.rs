...
/// Extra on-chain program ids that own pool/oracle/observation state accounts
/// but are NOT executable swap programs we load a `.so` for. They must still be
/// added to the Yellowstone owner-filter so every account they own streams into
/// the hot cache and stays live; otherwise those accounts are fetched once from
/// RPC and then go stale, making the affected pools fail local simulation while
/// the same routes land fine on-chain.
///
/// Verified against mainnet via getMultipleAccounts (executable=true, owned by
/// the upgradeable BPF loader):
///   - E2uCGJ4TtYyKPGaK57UMfbs9sgaumwDEZF1aAY6fF3mS: PancakeSwap V3 pool/oracle
///     state program. Owns observation accounts (e.g. 5VTjvi4…, space=168) that
///     PancakeSwap reads for pricing. Without live state these read stale and
///     the swap reverts in-sim only.
pub const OWNER_FILTER_ONLY_IDS: &[&str] =
...
/// `PROGRAMS` plus `OWNER_FILTER_ONLY_IDS` (pool-state programs without a `.so`).
pub fn all_program_ids() -> Vec<String> {
    PROGRAMS
        .iter()
        .map(|(id, _)| (*id).to_string())
        .chain(OWNER_FILTER_ONLY_IDS.iter().map(|id| (*id).to_string()))
        .collect()
}